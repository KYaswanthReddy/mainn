...
class Dis(nn.Module):
    def __init__(self, imdim=48, patch_size=13, layers = [], proj=128, num_classes=7, dropout=0.0):
        super().__init__()
        self.patch_size = patch_size
        self.n_channel = imdim
        self.layers_num = len(layers)
        ...
        for i in range(self.layers_num):
            self.sub_d.append(discriminator(inchannel=self.dims[i], outchannel=proj, num_classes=num_classes, patch_size=self.patch_size, dropout=dropout))
