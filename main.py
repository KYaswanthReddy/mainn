...
parser.add_argument('--sampling_mode', type=str, default='random', choices=['random', 'kmeans'],
                    help='Sampling mode for training data')
parser.add_argument('--dropout', type=float, default=0.0,
                    help='Dropout rate for discriminator/classifier fully connected layers')
...
def experiment(log_dir = ''):
    ...
    device = args.gpu
    hyperparams = {k: v for k, v in vars(args).items() if not (
        (k == 'dropout' and v == 0.0) or
        (k == 'weight_decay' and v == 0.0) or
        (k == 'label_smoothing' and v == 0.0) or
        (k == 'sampling_mode' and v == 'random')
    )}
    print(hyperparams)
    ...
    if args.sampling_mode == 'kmeans':
        ...
        for label, indices in class_indices.items():
            ...
            # Extract features (center pixel spectral signature) for all samples in this class
            feats = []
            for idx in indices:
                x, y = train_dataset.indices[idx]
                pixel = train_dataset.data[x, y]
                feats.append(pixel)
            
            X = np.array(feats)  # shape (num_samples, feature_dim)
            ...
            for c in range(samples_per_class):
                cluster_sample_indices = np.where(kmeans.labels_ == c)[0]
                if len(cluster_sample_indices) == 0:
                    distances = np.linalg.norm(X - kmeans.cluster_centers_[c], axis=1)
                    closest_idx_in_class = np.argmin(distances)
                    selected_indices.append(indices[closest_idx_in_class])
                else:
                    cluster_feats = X[cluster_sample_indices]
                    distances = np.linalg.norm(cluster_feats - kmeans.cluster_centers_[c], axis=1)
                    closest_idx_in_cluster = cluster_sample_indices[np.argmin(distances)]
                    selected_indices.append(indices[closest_idx_in_cluster])
    else:
        for label, indices in class_indices.items():
    ...
    d1 = Dis(imdim=N_BANDS, patch_size = hyperparams['patch_size'],layers = layers, proj=args.pro_dim, num_classes=num_classes, dropout=args.dropout).to(args.gpu)
    d2 = Dis(imdim=N_BANDS, patch_size = hyperparams['patch_size'],layers = layers, proj=args.pro_dim, num_classes=num_classes, dropout=args.dropout).to(args.gpu)
    ...
    D_net = discriminator(inchannel=N_BANDS, outchannel=args.pro_dim, num_classes=num_classes, patch_size=hyperparams['patch_size'], dropout=args.dropout).to(args.gpu)
    ...
    if args.sam_bool:
        ...
        D_sam = SAM(D_net.parameters(), D_opt, rho=args.sam_rho, lr=args.lr, momentum=0.9, **opt_kwargs)
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(D_sam.base_optimizer, T_max=args.max_epoch, eta_min=1e-4)
    else:
        D_opt = torch.optim.Adam(D_net.parameters(), lr=args.lr, **opt_kwargs)
...
def work():
    ...
    for i in range(repeat_time):
        ...
        mean_acc += acc
        mean_kappa += kappa
    print({k: v for k, v in vars(args).items() if not (
        (k == 'dropout' and v == 0.0) or
        (k == 'weight_decay' and v == 0.0) or
        (k == 'label_smoothing' and v == 0.0) or
        (k == 'sampling_mode' and v == 'random')
